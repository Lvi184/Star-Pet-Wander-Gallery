...
REGIONS = ["star-forest", "moon-lake", "crater", "crystal-cave", "cloud-peaks", "abyss", 
           "qingqiu", "kunlun", "donghai", "lingxu", "youdu", "xinghai"]
REGION_NAMES = {
    "star-forest": "星光森林",
    "moon-lake": "月牙湖",
    "crater": "陨石坑",
...
class WorldTickService:
    ...
    @classmethod
    async def generate_tick_events(cls, character: Character, tick_count: int) -> List[Dict]:
        events = []
        current_region = character.current_region or "star-forest"
        current_mood = character.mood
        current_energy = character.energy
        inventory = character.inventory or []

        for _ in range(tick_count):
            event = await cls._generate_single_event(
                character, current_region, current_mood, current_energy, inventory
            )
            events.append(event)
    ...
    async def _generate_single_event(
        cls, character: Character, current_region: str, mood: float, energy: float, inventory: List[str]
    ) -> Dict:
        event_type = cls._decide_event_type(character, mood, energy)
    ...
    @classmethod
    def _decide_event_type(cls, character: Character, mood: float, energy: float) -> str:
    ...
    @classmethod
    async def run_world_tick(cls):
        from services.websocket_manager import ws_manager
        
        async for session in get_db():
            await cls._check_heartbeat_timeout(session)

            result = await session.execute(
                select(Character)
                .filter(Character.controller_type == "agent")
                .filter(Character.status.in_(["alive", "normal"]))
            )
            agents = result.scalars().all()

            agent_controller = AgentController(session)

            for agent in agents:
                try:
                    action_result = await agent_controller.run_turn(agent.id)
                    if action_result.success:
                        agent.last_active = datetime.now()
                        await session.flush()
                        
                        if action_result.event_record:
                            event_record = action_result.event_record
                            event_data = {
                                "id": event_record.id,
                                "char_id": event_record.char_id,
                                "action_type": event_record.action_type,
                                "location": event_record.location,
                                "detail": event_record.detail,
                                "cause_chain": event_record.cause_chain,
                                "result": event_record.result,
                                "timestamp": event_record.timestamp.isoformat() if event_record.timestamp else None,
                                "created_at": event_record.created_at.isoformat() if event_record.created_at else datetime.now().isoformat(),
                            }
                            await ws_manager.broadcast_to_subscribers(agent.id, {
                                "type": "pet_event",
                                "pet_id": agent.id,
                                "event": event_data,
                            })
                    ...
                    import traceback
                    print(f"Agent {agent.id} tick error: {e}")
                    traceback.print_exc()

            await session.commit()

    @classmethod
    async def _check_heartbeat_timeout(cls, session: AsyncSession):
        result = await session.execute(
            select(Character)
            .filter(Character.controller_type == "player")
            .filter(Character.status.in_(["alive", "normal"]))
        )
        player_controlled = result.scalars().all()

        now = datetime.now()
        timeout = Character.HEARTBEAT_TIMEOUT_SECONDS

        for character in player_controlled:
            if character.last_player_heartbeat:
                elapsed = (now - character.last_player_heartbeat).total_seconds()
                if elapsed > timeout:
                    character.controller_type = "agent"
                    character.controller_version += 1
                    print(f"角色 {character.name} ({character.id}) 心跳超时，自动切换为AI控制")
            else:
                character.controller_type = "agent"
                character.controller_version += 1
                print(f"角色 {character.name} ({character.id}) 从未收到心跳，切换为AI控制")

        if player_controlled:
            await session.flush()

    @classmethod
    async def sync_offline_events(cls, char_id: str, last_online_time: datetime) -> Dict:
        async for session in get_db():
            result = await session.execute(select(Character).filter(Character.id == char_id))
            ...
            if offline_duration < cls.MIN_OFFLINE_SECONDS:
                return {
                    ...
                    "events_generated": 0,
                    "current_status": cls._get_character_status(character),
                    "events": [],
                }
            ...
            events = await cls.generate_tick_events(character, tick_count)

            await cls._save_events(session, char_id, events)
            await cls._update_character_state(session, character, events)
    ...
    @classmethod
    async def _save_events(cls, session: AsyncSession, char_id: str, events: List[Dict]):
        for idx, event_data in enumerate(events):
            event_id = f"event_{char_id}_{int(datetime.now().timestamp())}_{idx}"
            event = PetEvent(
                id=event_id,
                char_id=char_id,
                action_type=event_data["event_type"],
                location=event_data["location"],
                detail=event_data["detail"],
                cause_chain=[event_data.get("reason", "")],
                result={
    ...
    @classmethod
    async def _update_character_state(cls, session: AsyncSession, character: Character, events: List[Dict]):
        for event in events:
            character.mood = max(0, min(100, character.mood + event.get("mood_change", 0)))
            character.energy = max(0, min(100, character.energy + event.get("energy_change", 0)))
            character.current_region = event.get("location", character.current_region)

            if event.get("items_found"):
                if character.inventory is None:
                    character.inventory = []
                character.inventory.extend(event["items_found"])

        character.last_active = datetime.now()
        await session.commit()

    @classmethod
    def _get_character_status(cls, character: Character) -> Dict:
        return {
            "mood": character.mood,
    ...
    @classmethod
    async def get_character_events(cls, char_id: str, limit: int = 20) -> List[Dict]:
        async for session in get_db():
            result = await session.execute(
                select(PetEvent)
                .filter(PetEvent.char_id == char_id)
